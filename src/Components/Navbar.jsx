...
function Navbar({ activeSection, scrollToSection, menuOpen, setMenuOpen, scrollY, heroRef, aboutRef, projectsRef, contactRef }) {
  return (
    <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${scrollY > 50 ? 'bg-slate-950/95 backdrop-blur-md shadow-lg shadow-blue-500/10' : ''}`}>
      <div className="max-w-7xl mx-auto px-6 lg:px-20 py-6 flex justify-between items-center">
        ...
        <div className="hidden lg:flex gap-8">
          {['home', 'about', 'projects', 'contact'].map((section) => (
            <button
              key={section}
              className={`relative px-4 py-2 capitalize font-medium transition-all ${
